...
// =======================================================
// Viewport height & FV sync
// =======================================================
let vh = window.innerHeight;
const kv2fv = () => {
  const $kv = $('#kv');
  if ($kv.length) $('#fv').css({ height: $kv.innerHeight() });
};
...
// =======================================================
// Introduction Showcase Swiper
// =======================================================
if ($('#js-introShowcaseSwiper').length) {
  const introShowcaseImages = Array.isArray(window.SITE_DATA?.portfolio?.introShowcaseImages)
    ? window.SITE_DATA.portfolio.introShowcaseImages
    : [];
  ...
  const introShowcaseSwiper = new Swiper('#js-introShowcaseSwiper', {
    ...
    slidesPerView: 3,
    effect: 'coverflow',
    coverflowEffect: { rotate: 34, stretch: 8, depth: 220, modifier: 1.05, slideShadows: false },
    spaceBetween: 14,
    breakpoints: {
      769: {
        coverflowEffect: { rotate: 34, stretch: 8, depth: 220, modifier: 1.05, slideShadows: false },
      ...
      0: {
        coverflowEffect: { rotate: 26, stretch: 2, depth: 130, modifier: 1, slideShadows: false },
        spaceBetween: 8,
      },
    },
    autoplay: { delay: 3600, disableOnInteraction: false, pauseOnMouseEnter: true },
    on: {
      init:        function(s) { $introSwitcherItems.removeClass('is-active').eq(s.realIndex).addClass('is-active'); },
      slideChange: function(s) { $introSwitcherItems.removeClass('is-active').eq(s.realIndex).addClass('is-active'); },
    },
  });
...
if ($('#js-bnr').length) {
  getPost('pages', 131).done(function(data) {
    ...
    const chunks = [];
    for (let i = 0; i < bnrLists.length; i += 2) chunks.push(bnrLists.slice(i, i + 2));
...
const navLinkSel    = '.nav__itemLink, .gnav__itemLink';
...
const setActiveNavById = (id) => {
  if (!id) return;
  document.querySelectorAll(navLinkSel).forEach((link) => {
...
const applyActiveSection = (id) => {
  ...
  sectionIds.forEach(sid => $body.removeClass(`is-${sid}`));
  $body.addClass(`is-${id}`);
  setActiveNavById(id);
  currentSection = id;
  ...
  if (id === 'news') {
    kvOrbitHideTimer = setTimeout(() => {
      if (currentSection === 'news') $body.addClass('is-kv-orbit-force-hide');
    }, 500);
  }
};
...
const sectionObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    ...
    sectionVis.set(entry.target.id, { isIntersecting: entry.isIntersecting, ratio: entry.intersectionRatio || 0 });
  });

  let nextId = '', bestRatio = -1;
  sectionIds.forEach(id => {
    const state = sectionVis.get(id);
    if (!state || !state.isIntersecting) return;
    if (state.ratio > bestRatio || (state.ratio === bestRatio && nextId && sectionIds.indexOf(id) > sectionIds.indexOf(nextId))) {
      bestRatio = state.ratio;
      nextId = id;
    }
  });

  if (nextId) applyActiveSection(nextId);
