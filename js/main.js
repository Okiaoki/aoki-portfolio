...
(() => {
  ...
  // ===== Opening overlay (first view per tab session only) =====
  const openingEl = document.getElementById('opening');
  if (openingEl) {
    const OPENING_KEY = 'opening_shown_v1';
    const OPENING_MS = 1950;
    const OPENING_EXIT_MS = 920;
    ...
    function exitOpening(immediate = false) {
      if (settled) return;
      if (immediate) {
        cleanupOpening();
        return;
      }
      openingEl.classList.add('is-exit');
      openingEl.setAttribute('aria-hidden', 'true');
      exitTimer = window.setTimeout(cleanupOpening, OPENING_EXIT_MS);
    }

    if (reducedMotion || hasShownOpening()) {
      cleanupOpening();
    } else {
      markShownOpening();
      openingEl.classList.add('is-active');
      openingEl.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
      document.addEventListener('visibilitychange', onVisibilityChange);
      startTimer = window.setTimeout(() => exitOpening(false), OPENING_MS);
    }
  }
